...
--[労働者]----------
  ...
  時給換算手取り：{$format_yen($振込額/($定時*$勤務日数))}
  --------------------------
...
---[企業]---------------
    ...
    　労働者からみた時給: {$format_yen($振込額/($定時*$勤務日数))}
...
会社が払う時給　：{$format_yen($会社から見た時給)}
求人票の時給　　：{$format_yen($時給)}
手取り（時給）　：{$format_yen($振込額/($定時*$勤務日数))}
国が徴収する時給：{$format_yen($会社から見た時給-$振込額/($定時*$勤務日数))}
